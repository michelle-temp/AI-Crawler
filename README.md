...
**Workers Cache in front of the Worker — configured entirely with HTTP headers.** `[cache] enabled = true` in [wrangler.toml](wrangler.toml) puts Cloudflare's cache *before* the Worker: fresh hits are answered without invoking it. See [ARCHITECTURE.md](ARCHITECTURE.md).
